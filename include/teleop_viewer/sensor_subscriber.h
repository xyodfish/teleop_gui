...
#include <galbot/singorix_proto/singorix_info.pb.h>
#include <galbot/singorix_proto/singorix_error.pb.h>
...
class SensorSubscriber {
   public:
    ...
    bool startWbcInfo(const std::string& topic);
    bool startRobotErrors(const std::string& topic);
    ...
    std::vector<WbcGroupErrorSample> latestWbcGroupErrors() const;
    std::vector<RobotErrorSample> latestRobotErrors() const;
    int latestWbcJointNameCount() const;
    int latestWbcStatePosCount() const;
    ...
    uint64_t wbcMessageCount() const;
    double wbcMessageAgeSec() const;
    bool wbcHasRecentData(double timeout_sec) const;
    uint64_t errorMessageCount() const;
    double errorMessageAgeSec() const;
    bool errorHasRecentData(double timeout_sec) const;
   ...
   private:
    ...
    void onWbcInfoMessage(const std::shared_ptr<galbot::singorix_proto::WBCInfo>& msg);
    void onErrorMessage(const std::shared_ptr<galbot::singorix_proto::SingoriXError>& msg);

    std::string topic_;
    std::string joy_topic_;
    std::string state_topic_;
    std::string wbc_topic_;
    ...
    std::shared_ptr<galbot::embosa::SerializationReader<galbot::singorix_proto::WBCInfo>> wbc_reader_;
    std::shared_ptr<galbot::embosa::SerializationReader<galbot::singorix_proto::SingoriXError>> error_reader_;
    ...
    std::atomic<uint64_t> wbc_message_count_{0};
    std::atomic<long long> wbc_last_msg_ns_{0};
    std::atomic<uint64_t> error_message_count_{0};
    std::atomic<long long> error_last_msg_ns_{0};
