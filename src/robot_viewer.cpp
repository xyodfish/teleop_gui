    ...
    void updateTransforms() {
        ...
        traverse = [&](urdf::LinkConstSharedPtr link, const glm::mat4& parent_transform) {
            ...
            for (auto& child_link : link->child_links) {
                ...
                // 调试打印
                std::cout << "[Transform] Link: " << child_link->name << " (via joint " << joint->name << ") "
                          << "origin=(" << p.x << "," << p.y << "," << p.z << ") "
                          << "rpy=(" << roll << "," << pitch << "," << yaw << ") "
                          << "pos=(" << child_transform[3][0] << "," << child_transform[3][1] << "," << child_transform[3][2] << ")"
                          << std::endl;
...
int main() {
    ...
    Robot robot;
    robot.loadURDF(
        "/home/yuxia/Workspace/SingoriX/OmniLink/singorix_teleop_gui/galbot_description/galbot_one_golf_description/galbot_one_golf.urdf");

    // 摄像机（保留）
    camera = Camera(glm::vec3(0.0f, 0.0f, 2.0f), glm::vec3(0.0f, 0.0f, 1.0f), -90.0f, 0.0f);
